...
describe('QRCode', () => {
    describe('#render', () => {
        it('uses medium size by default', () => {
            ...
            const wrapper = mount(React.createElement(QRCode, { data: 'avervr24v' }));
            const { src } = wrapper.find('img').first().props();
            expect(src).toContain(mdSize);
        });
        it('uses medium size if size prop is md', () => {
            const mdSize = '118x118';
            const wrapper = mount(React.createElement(QRCode, { data: 'avervr24v', dimensions: 'md' }));
        ...
        it('should contain default props', () => {
            ...
            const wrapper = mount(React.createElement(QRCode, { data: text, dimensions: size, alt: alternative }));
        ...
        it('renders correctly data with empty spaces', () => {
            ...
            const wrapper = mount(React.createElement(QRCode, { data: address, dimensions: "sm" }));
            const { src } = wrapper.find('img').first().props();
            expect(src).toContain(encodeURI(address));
        });
        it('renders correctly', () => {
            const tree = renderer
                .create(React.createElement(QRCode, { data: "sevaerverv343", dimensions: "sm" }))
                .toJSON();
