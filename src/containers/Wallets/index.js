import { Button, Decimal, DepositCrypto, DepositFiat, FilterInput, TabPanel, WalletList, } from '@components/components';
...
class WalletsComponent extends React.Component {
    constructor(props) {
        ...
        this.toggleConfirmModal = (amount, total, beneficiary_id, beneficiary_details, otpCode) => {
            this.setState((state) => ({
                ...
                total: total ? total : 0,
                beneficiary_details: beneficiary_details ? beneficiary_details : null,
            }));
        };
        ...
        this.handleWithdraw = () => {
            const { selectedWalletIndex, otpCode, amount, beneficiary_id, } = this.state;
        ...
        this.renderSingle = () => {
            ...
            const locked = (React.createElement("div", null,
                React.createElement("div", { className: "base-wallet-item__amount-locked" },
                    React.createElement(FormattedMessage, { id: "page.body.wallets.locked" })),
                React.createElement("span", { className: "base-wallet-item__balance-locked" },
            ...
            const displayBalance = (React.createElement("div", null,
                ...
                "\u00A0",
                React.createElement("span", { className: "base-wallet-item__balance-amount" },
                    React.createElement(Decimal, { fixed: selectedFixed }, stringBalance))));
            return (React.createElement("div", { className: "base-wallet-item__single" },
                React.createElement("span", { className: `base-wallet-item__icon-code base-crypto-font-${formattedCurrency}` }),
                React.createElement("div", { className: "base-wallet-item__single-balance" },
                    locked,
                    displayBalance)));
        };
        this.handleOnCopy = () => {
        ...
        this.isOtpDisabled = () => {
            return (React.createElement(React.Fragment, null,
                React.createElement("p", { className: "parent-wallet__enable-2fa-message" }, this.translate('page.body.wallets.tabs.withdraw.content.enable2fa')),
                React.createElement(Button, { className: "parent-wallet__button-2fa", label: this.translate('page.body.wallets.tabs.withdraw.content.enable2faButton'), onClick: this.redirectToEnable2fa })));
        };
    ...
    render() {
        ...
        const { total, beneficiary_id, beneficiary_details, selectedWalletIndex, filteredWallets, withdrawSubmitModal, withdrawConfirmModal, } = this.state;
        ...
        return (React.createElement("div", { className: "parent-wallet parent-container" },
            React.createElement(FilterInput, { filter: this.handleFilter, onFilter: this.searchCallback, data: formattedWallets }),
            React.createElement("p", { className: "parent-wallet__no-results" }, maybeNoResults),
            React.createElement("div", { className: `parent-wallet__tabs-content ${!historyList.length && 'parent-wallet__tabs-content-height'}` },
                ...
                React.createElement("div", { className: "parent-wallet__tabs" }, maybeSelectedTab)),
            ...
            React.createElement(ModalWithdrawConfirmation, { show: withdrawConfirmModal, amount: total, currency: selectedCurrency, beneficiary_id: beneficiary_id, beneficiary_details: beneficiary_details, onSubmit: this.handleWithdraw, onDismiss: this.toggleConfirmModal })));
    }
    consist(a, b) {
    ...
    renderWithdraw(withdrawProps, type) {
        ...
        if (type === 'fiat') {
            return (
                <div>
                    <p className="base-deposit-fiat__title">{this.translate('page.body.wallets.tabs.deposit.fiat.admin')}</p>
                    <p className="base-deposit-fiat__title">{this.translate('page.body.wallets.tabs.deposit.fiat.email')}: fund@auroratokens.com</p>
                </div>
            )
        }
        return (React.createElement(React.Fragment, null,
            this.renderSingle(),
            walletsError && React.createElement("p", { className: "parent-wallet__error" }, walletsError.message),
