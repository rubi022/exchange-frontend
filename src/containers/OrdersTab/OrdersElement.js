import { CloseButton, Decimal, History, Pagination } from '@components/components';
...
class OrdersComponent extends React.PureComponent {
    constructor() {
        ...
        this.renderOrdersHistoryRow = item => {
            ...
            return [
                ...
                marketName,
                React.createElement(Decimal, { key: id, fixed: currentMarket.price_precision }, actualPrice),
                React.createElement(Decimal, { key: id, fixed: currentMarket.amount_precision }, remaining_volume),
                React.createElement(Decimal, { key: id, fixed: currentMarket.amount_precision }, executed_volume),
                React.createElement(Decimal, { key: id, fixed: currentMarket.amount_precision }, remaining_volume),
                React.createElement(Decimal, { key: id, fixed: currentMarket.amount_precision }, costRemaining.toString()),
                status,
        ...
        this.setOrderStatus = (status) => {
            switch (status) {
                case 'done':
                    return (React.createElement("span", { className: "parent-history-elem-executed" },
                        React.createElement(FormattedMessage, { id: `page.body.openOrders.content.status.done` })));
                case 'cancel':
                    return (React.createElement("span", { className: "parent-history-elem-canceled" },
                        React.createElement(FormattedMessage, { id: `page.body.openOrders.content.status.cancel` })));
                case 'wait':
                    return (React.createElement("span", { className: "parent-history-elem-opened" },
    ...
    render() {
        ...
        return (React.createElement("div", { className: `parent-history-elem ${list.length ? '' : 'parent-history-elem-empty'}` },
            list.length ? this.renderContent() : null,
            !list.length && !fetching ? React.createElement("p", { className: "parent-history-elem__empty" }, emptyMsg) : null));
    }
}
